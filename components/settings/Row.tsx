  ...
  const inner = (
    <div
      className={[
        ...
        "border-b border-[rgb(var(--app-border))]/70 last:border-b-0",
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="truncate text-[15px] font-medium text-[rgb(var(--app-text))]">
          {title}
        </div>
        {subtitle ? (
          <div className="truncate text-sm text-[rgb(var(--app-text-muted))]">{subtitle}</div>
        ) : null}
      </div>

      {/* right accessory */}
      {end ? (
        <div className="ml-2 shrink-0 text-sm text-[rgb(var(--app-text-muted))]">{end}</div>
      ) : href ? (
