...
export default function VerificationSteps({
  ...
  return (
    <div className="mt-6">
      <h2 className="mb-3 text-2xl font-bold text-[rgb(var(--app-text))]">Verification steps</h2>

      <div className="overflow-hidden rounded-2xl border border-[rgb(var(--app-border))]">
        {sections.map((s, idx) => {
          const isOpen = !!open[s.id];
          return (
            <div
              key={s.id}
              className="border-b last:border-0 border-[rgb(var(--app-border))]"
            >
              <button
                type="button"
              ...
              >
                <span className="flex items-center gap-3 text-left">
                  ...
                  <span className="text-base font-medium text-[rgb(var(--app-text))]">
                    {s.title}
                  </span>
                </span>
                <span className="flex items-center gap-3">
                  ...
                  <ChevronDown
                    className={`h-5 w-5 text-[rgb(var(--app-text-muted))] transition ${
                      isOpen ? "rotate-180" : ""
                    }`}
                  />
                </span>
              ...
              {isOpen && (
                <div className="px-4 pb-5 pt-2 bg-[rgb(var(--app-surface))]/60">
                  {s.content}
                </div>
              )}
            </div>
