...
export default function MobileSidebar({ open, onClose }: Props) {
  ...
  return (
    <>
      ...
      {/* drawer panel */}
      <aside
        className={`fixed left-0 top-16 bottom-0 z-50 w-[86%] max-w-[320px] border-r border-neutral-900 bg-[rgb(var(--app-surface))] transition-transform md:hidden ${
        ...
        <div className="h-full overflow-y-auto px-2 pb-4 pt-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          ...
          <div className="mt-3">
            <Link
              href="#"
              onClick={onClose}
              className="block w-full rounded-xl border border-[rgb(var(--app-border))] bg-[rgb(var(--app-surface-2))]/70 px-3 py-2 text-center text-sm text-[rgb(var(--app-text-soft))] hover:bg-neutral-900"
            >
              Close
            </Link>
          </div>
