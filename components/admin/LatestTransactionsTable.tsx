...
export default function LatestTransactionsTable({ rows }: { rows: Txn[] }) {
  return (
    <div className="rounded-2xl bg-[rgb(var(--app-surface))] border border-white/5 p-5">
      <p className="text-sm text-[rgb(var(--app-text-muted))] mb-3">Latest Transactions</p>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="text-[rgb(var(--app-text-muted))] border-b border-white/5">
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="border-b border-white/5 last:border-0">
                ...
                <td className="py-3 text-[rgb(var(--app-text-muted))]">
