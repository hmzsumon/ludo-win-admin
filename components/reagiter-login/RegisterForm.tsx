...
const RegisterForm: React.FC<{ onSuccess?: () => void }> = ({ onSuccess }) => {
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      ...
      <details className="rounded-lg border border-[rgb(var(--app-border))] p-3 open:bg-neutral-900/40">
      ...
      {/* ── declaration ─────────────────────────────────────── */}
      <label className="flex items-start gap-3 text-sm text-[rgb(var(--app-text-soft))]">
        <input
          type="checkbox"
