...
export default function DepositItemCard({ item }: { item: DepositItem }) {
  ...
  const tint = item.colorClass || "text-[rgb(var(--app-text-soft))]";
  ...
  return (
    <div
      className={[
        ...
        locked
          ? "border-neutral-850 bg-[rgb(var(--app-surface))]/80"
          : "border-emerald-800/40 bg-[rgb(var(--app-surface))]/60 ring-1 ring-emerald-800/20",
      ].join(" ")}
    >
      {/* ── header: logo + name ─────────────────────────────── */}
      <div className="mb-3 flex items-center gap-3">
        <div className="relative h-9 w-9 overflow-hidden rounded-lg border border-[rgb(var(--app-border))] bg-gray-200">
          {item.Icon ? (
        ...
        <div className="flex-1">
          <div className="font-medium text-[rgb(var(--app-text))]">{item.name}</div>
          <div className="text-xs text-[rgb(var(--app-text-muted))]">{item.processing}</div>
        </div>

        {/* ── status badge ──────────────────────────────────── */}
        {locked ? (
          <span className="inline-flex items-center gap-1 rounded-full border border-[rgb(var(--app-border))] bg-neutral-900/70 px-2 py-0.5 text-[10px] font-medium text-[rgb(var(--app-text-muted))]">
      ...
      <div className="space-y-1 text-xs">
        <div className="flex items-center justify-between">
          <span className="text-[rgb(var(--app-text-muted))]">Fee</span>
          <span className="font-medium text-neutral-200">{item.fee}</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-[rgb(var(--app-text-muted))]">Limits</span>
      ...
      <div className="mt-3">
        {locked ? (
          <button
            type="button"
            disabled
            className="w-full cursor-not-allowed rounded-lg border border-[rgb(var(--app-border))] bg-[rgb(var(--app-surface-2))]/70 px-3 py-2 text-sm text-[rgb(var(--app-text-muted))]"
      ...
      {item.tags && item.tags.length > 0 && (
        <div className="mt-3 flex flex-wrap gap-1.5">
          {item.tags.map((t) => (
            <span
              key={t}
              className="rounded-md border border-[rgb(var(--app-border))] bg-[rgb(var(--app-surface-2))]/70 px-2 py-0.5 text-[10px] text-[rgb(var(--app-text-muted))]"
            >
              {t}
            </span>
          ))}
