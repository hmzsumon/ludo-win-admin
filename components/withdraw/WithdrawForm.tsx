...
const FieldLabel: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <label className="mb-1 block text-sm text-[rgb(var(--app-text-soft))]">{children}</label>
);
const SectionTitle: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => <h3 className="text-sm font-semibold text-[rgb(var(--app-text))]">{children}</h3>;
  ...
  return (
    <div className="mt-6 rounded-xl border border-[#00493B] bg-[#031A15] p-4">
      ...
      <div className="mt-3">
        ...
        <div className="relative">
          ...
          <button
            type="button"
            onClick={() => setShow((v) => !v)}
            className="absolute right-2 top-1/2 -translate-y-1/2 rounded p-1 hover:bg-[rgb(var(--app-surface-3))]/80"
      ...
      <button
        ...
        className="mt-4 w-full rounded-lg bg-white/20 py-3 font-medium text-[rgb(var(--app-text))] transition
