...
const FeaturesGrid: React.FC = () => (
  <section id="features" className="bg-[rgb(var(--app-surface))] py-14 sm:py-20">
    <Container>
      <SectionTitle
        ...
        {items.map((f, idx) => (
          <Card key={idx}>
            <div className="flex items-start gap-4">
              ...
              <div>
                <h3 className="font-semibold text-[rgb(var(--app-text))]">{f.title}</h3>
                <p className="mt-1 text-sm text-[rgb(var(--app-text-soft))]">{f.desc}</p>
              </div>
            </div>
          </Card>
        ))}
