    ...
    <Link href={game.link}>
      ...
      >
        ...
        <div className="absolute inset-x-0 bottom-2 flex justify-center">
          <div className="rounded-full bg-black/70 px-3 py-1 text-[11px] font-medium text-[rgb(var(--app-text))] backdrop-blur-sm">
  ...
  return (
    <div className="mb-4 flex items-center justify-between">
      ...
      <div className="flex items-center gap-2">
        ...
        >
          <ChevronLeft className="h-5 w-5 text-[rgb(var(--app-text))]" />
        </button>
        <button
          aria-label="next"
          onClick={onNext}
          className="grid h-9 w-9 place-items-center rounded-xl bg-emerald-900/60 ring-1 ring-white/10 hover:bg-emerald-800/70"
        >
          <ChevronRight className="h-5 w-5 text-[rgb(var(--app-text))]" />
        </button>
      </div>
    </div>
  );
...
export default function HotGamesMenu({
  ...
  return (
    <section className="w-full rounded-3xl bg-gradient-to-b from-emerald-950 via-emerald-900 to-emerald-950 p-4 text-[rgb(var(--app-text))]">
      <SectionHeader />
