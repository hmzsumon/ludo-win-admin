...
export default function ItemRow({
  ...
  const base = (
    <div
      ...
      ${
        active
          ? "bg-neutral-800/60 text-[rgb(var(--app-text))]"
          : "text-[rgb(var(--app-text-soft))] hover:bg-neutral-900 hover:text-[rgb(var(--app-text))]"
      }`}
    >
            ...
            {childrenItems.map((c) => (
              <li key={c.href}>
                <Link
                  href={c.href}
                  onClick={onNavigate}
                  className={`block rounded-lg px-3 py-2 text-sm text-[rgb(var(--app-text-soft))] hover:bg-neutral-900 hover:text-[rgb(var(--app-text))]`}
                >
                  <div className="flex items-center justify-between">
                    <span>{c.label}</span>
                    {c.sublabel ? (
                      <span className="ml-3 text-[10px] text-[rgb(var(--app-text-muted))]">
                        {c.sublabel}
                      </span>
                    ) : null}
                  </div>
