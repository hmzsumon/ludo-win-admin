...
export default function DesktopSidebar() {
  ...
  const Group = ({ item }: { item: NavItem }) => {
    ...
    // row classes
    const row =
      "flex items-center rounded-xl px-3 py-2 text-sm text-[rgb(var(--app-text-soft))] hover:bg-[rgb(var(--app-surface-2))] hover:text-[rgb(var(--app-text))] transition-colors";
          ...
          <div className="ml-9 space-y-1 py-1">
            {item.children!.map((c) => (
              <Link
                key={c.label}
                href={c.href}
                className="block rounded-lg px-2 py-1.5 text-sm text-[rgb(var(--app-text-soft))] hover:bg-[rgb(var(--app-surface-2))] hover:text-[rgb(var(--app-text))] transition-colors"
              >
  ...
  return (
    <nav
      className={[
        "sticky top-16 h-[calc(100dvh-4rem)] border-r border-[rgb(var(--app-border))]",
        "bg-[rgb(var(--app-surface))]/95 backdrop-blur",
        ...
        "[&::-webkit-scrollbar-thumb]:bg-[rgb(var(--app-surface-3))] hover:[&::-webkit-scrollbar-thumb]:bg-[rgb(var(--app-text-muted))]",
      ].join(" ")}
    >
      {/* top section */}
      ...
      {!collapsed && (
        <div className="mt-3 rounded-xl border border-[rgb(var(--app-border))] bg-[rgb(var(--app-surface-2))]/70 p-3">
          <div className="text-sm font-medium">{INVITE_CARD.title}</div>
          <Link
            href={INVITE_CARD.href}
            className="mt-2 inline-flex rounded-lg bg-gradient-to-r from-emerald-400 to-cyan-500 px-3 py-1.5 text-xs font-semibold text-[rgb(var(--app-brand-contrast))]"
          >
            Open
          </Link>
        </div>
      ...
      <div className="sticky -bottom-2 flex w-full justify-center bg-[rgb(var(--app-surface-2))]/70">
        <button
          onClick={() => dispatch(toggleSidebar())}
          className="flex h-9 w-9 items-center justify-center rounded-full border border-[rgb(var(--app-border))] bg-[rgb(var(--app-surface))] text-[rgb(var(--app-text-muted))] hover:border-[rgb(var(--app-text-muted))] hover:text-[rgb(var(--app-text))]"
