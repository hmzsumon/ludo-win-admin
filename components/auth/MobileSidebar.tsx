...
export default function MobileSidebar({ open, onClose }: Props) {
  ...
  return (
    <>
      ...
      <div
        className={`fixed inset-x-0 top-16 bottom-0 z-40 bg-[rgb(var(--app-overlay))] transition-opacity md:hidden ${
          open
      ...
      {/* ── drawer under header ────────────────────────────────── */}
      <aside
        className={`fixed left-0 top-16 bottom-0 z-50 w-[86%] max-w-[320px] border-r border-[rgb(var(--app-border))] bg-[rgb(var(--app-surface))] transition-transform md:hidden ${
      ...
      >
        ...
        <div className="h-full overflow-y-auto px-2 pb-4 pt-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {/* ── user block ──────────────────────────────────────── */}
          <div className="mb-3 rounded-xl border border-[rgb(var(--app-border))] bg-[rgb(var(--app-surface-2))]/70 p-3">
            <div className="flex items-center gap-3">
              <div>
                <div className="text-sm font-semibold text-[rgb(var(--app-text))]">
                  {user?.name}
                </div>
                <div className="text-xs text-[rgb(var(--app-text-muted))]">{user?.email}</div>
              </div>
            </div>
            ...
            <div className="mt-3 space-y-1">
              <button
                type="button"
                className="block w-full rounded-lg px-2 py-2 text-left text-sm hover:bg-[rgb(var(--app-surface-2))]"
          ...
          <div className="mb-3 rounded-xl border border-[rgb(var(--app-border))] bg-[rgb(var(--app-surface-2))]/70">
            <button
              ...
              className="flex w-full items-center justify-between px-3 py-3 text-sm text-[rgb(var(--app-text-soft))]"
            >
              <span className="flex items-center gap-2 font-medium">
                <span className="inline-flex items-center rounded-md px-2 py-1 ring-1 ring-[rgb(var(--app-border))]">
                  {hideBalance
                    ? "••••"
            ...
            {balanceOpen && (
              <div className="space-y-3 border-t border-[rgb(var(--app-border))] p-3 pt-2">
                ...
                <div className="flex items-center justify-between">
                  <code className="text-xs text-[rgb(var(--app-text-soft))]">
                    {user?.customerId}
                  </code>
                  <button
                    type="button"
                    ...
                    <div className="ml-9 space-y-1 py-1">
                      {i.children!.map((c) => (
                        <Link
                          ...
                          href={c.href}
                          onClick={onClose}
                          className="block rounded-lg px-2 py-1.5 text-sm text-[rgb(var(--app-text-soft))] hover:bg-neutral-900 hover:text-[rgb(var(--app-text))]"
                        >
