    ...
    <form onSubmit={handleSubmit} className="mx-auto max-w-md space-y-4">
      ...
      {hasPin && (
        <label className="block text-sm">
          <span className="text-[rgb(var(--app-text-muted))]">Current PIN</span>
          <input
            inputMode="numeric"
            pattern="\d*"
            maxLength={6}
            ...
            className="mt-1 w-full rounded-lg border border-[rgb(var(--app-border))] bg-neutral-900 px-3 py-2 outline-none focus:ring-2 focus:ring-emerald-600/40"
            placeholder="••••••"
          />
        </label>
      )}
      ...
      {/* ── new PIN ──────────────────────────────────────────── */}
      <label className="block text-sm">
        <span className="text-[rgb(var(--app-text-muted))]">New PIN (6 digits)</span>
        <input
          inputMode="numeric"
          pattern="\d*"
          maxLength={6}
      ...
      {/* ── confirm PIN ──────────────────────────────────────── */}
      <label className="block text-sm">
        <span className="text-[rgb(var(--app-text-muted))]">Confirm New PIN</span>
        <input
          inputMode="numeric"
          pattern="\d*"
          maxLength={6}
